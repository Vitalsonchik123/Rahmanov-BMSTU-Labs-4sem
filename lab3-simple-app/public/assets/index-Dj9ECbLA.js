(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=class{constructor(e){this.parent=e}getHTML(){return`
        ...
        `).join(``),document.querySelectorAll(`.btn-delete-order`).forEach(e=>{e.addEventListener(`click`,t=>{let n=parseInt(e.getAttribute(`data-id`));this.deleteOrder(n)})}))}goToHome(){new l(this.parent).render()}render(){this.parent.innerHTML=``,new e(this.parent).render(this.goToHome.bind(this),()=>{});let t=this.getHTML();this.parent.insertAdjacentHTML(`beforeend`,t);let n=document.getElementById(`add-order-button`);n&&n.addEventListener(`click`,()=>this.addOrder()),this.renderOrders()}},a=new class{get(e,t){let n=new XMLHttpRequest;n.open(`GET`,e),n.send(),n.onreadystatechange=()=>{n.readyState===4&&this._handleResponse(n,t)}}post(e,t,n){let r=new XMLHttpRequest;r.open(`POST`,e),r.setRequestHeader(`Content-Type`,`application/json`),r.send(JSON.stringify(t)),r.onreadystatechange=()=>{r.readyState===4&&this._handleResponse(r,n)}}patch(e,t,n){let r=new XMLHttpRequest;r.open(`PATCH`,e),r.setRequestHeader(`Content-Type`,`application/json`),r.send(JSON.stringify(t)),r.onreadystatechange=()=>{r.readyState===4&&this._handleResponse(r,n)}}delete(e,t){let n=new XMLHttpRequest;n.open(`DELETE`,e),n.send(),n.onreadystatechange=()=>{n.readyState===4&&t(null,n.status)}}_handleResponse(e,t){try{t(e.responseText?JSON.parse(e.responseText):null,e.status)}catch(n){console.error(`Ошибка парсинга JSON:`,n),t(null,e.status)}}},o=new class{constructor(){this.baseUrl=`http://localhost:3000`}getStocks(e=``){let t=`${this.baseUrl}/stocks`;return e&&(t+=`?search=${encodeURIComponent(e)}`),t}getStockById(e){return`${this.baseUrl}/stocks/${e}`}createStock(){return`${this.baseUrl}/stocks`}removeStockById(e){return`${this.baseUrl}/stocks/${e}`}updateStockById(e){return`${this.baseUrl}/stocks/${e}`}},s=class{constructor(e,t){this.parent=e,this.id=parseInt(t)}get pageRoot(){return document.getElementById(`product-page`)}getHTML(){return`
        ...
        `}getData(){a.get(o.getStockById(this.id),(e,t)=>{if(t===200&&e)this.renderData(e);else{console.error(`Ошибка загрузки карточки:`,t);let e=document.getElementById(`product-info-container`);e&&(e.innerHTML=`<div class="alert alert-danger">Не удалось загрузить данные акции</div>`)}})}renderData(e){let t=document.getElementById(`product-info-container`),i=document.getElementById(`three-model-container`);!t||!i||(new n(t).render(e),new r(i,e.modelPath||`./models/computer.glb`).render())}goToHome(){new l(this.parent).render()}goBack(){new l(this.parent).render()}goToOrders(){new i(this.parent).render()}render(){this.parent.innerHTML=``,new e(this.parent).render(this.goToHome.bind(this),this.goToOrders.bind(this));let t=this.getHTML();this.parent.insertAdjacentHTML(`beforeend`,t);let n=document.getElementById(`back-button`);n&&n.addEventListener(`click`,()=>this.goBack()),this.getData()}},c=class{constructor(e,t=null){this.parent=e,this.id=t?parseInt(t):null,this.stock=null}getHTML(){return`
        ...
        `}loadData(){return this.id?new Promise((e,t)=>{a.get(o.getStockById(this.id),(n,r)=>{r===200&&n?(this.stock=n,e(n)):t(Error(`Не удалось загрузить данные`))})}):Promise.resolve()}populateForm(){this.stock&&(document.getElementById(`title`).value=this.stock.title||``,document.getElementById(`text`).value=this.stock.text||``,document.getElementById(`src`).value=this.stock.src||``,document.getElementById(`category`).value=this.stock.category||``,document.getElementById(`discount`).value=this.stock.discount??0,document.getElementById(`promoCodes`).value=(this.stock.promoCodes||[]).join(`, `))}getFormData(){return{title:document.getElementById(`title`).value,text:document.getElementById(`text`).value,src:document.getElementById(`src`).value,category:document.getElementById(`category`).value,discount:parseInt(document.getElementById(`discount`).value)||0,promoCodes:document.getElementById(`promoCodes`).value.split(`,`).map(e=>e.trim()).filter(e=>e)}}save(e){e.preventDefault();let t=this.getFormData(),n=this.id?o.updateStockById(this.id):o.createStock();a[this.id?`patch`:`post`](n,t,(e,t)=>{t===200||t===201?new l(this.parent).render():alert(`Ошибка сохранения`)})}cancel(){new l(this.parent).render()}async render(){this.parent.innerHTML=``,new e(this.parent).render(()=>{new l(this.parent).render()},()=>{});let t=this.getHTML();this.parent.insertAdjacentHTML(`beforeend`,t),this.id&&(await this.loadData(),this.populateForm()),document.getElementById(`edit-form`).addEventListener(`submit`,e=>this.save(e)),document.getElementById(`cancel-btn`).addEventListener(`click`,()=>this.cancel())}},l=class{constructor(e){this.parent=e,this.products=[],this.filteredProducts=[]}get pageRoot(){return document.getElementById(`main-page`)}getHTML(){return`
            <div id="main-page">
                <div class="container">
                    <div class="row mb-4">
                        <div class="col-md-5">
